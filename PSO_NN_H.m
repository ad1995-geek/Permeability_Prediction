...
clear all 
rand('seed',45) 
data = xlsread('Core & Logs_BED_modified.xlsx','Sheet2');  
...
%%
[trainInd,testInd,valInd]=dividerand(95,0.65,0.35,0.0);   
...
plot(linspace(0,1000,10),linspace(0,1000,10),'black--');
xlim([0 1000]);
ylim([0 1000]);
legend('Train','Test','Location','northwest');
xlabel('observed','FontWeight','Bold');
ylabel('Predicted','FontWeight','Bold');
title('Horizontal Permeability Prediction by PSO-NN','FontWeight','Bold');
%exportgraphics(gcf,'Regression plot(H).jpg','Resolution',300);

%% sampling plot
figure(2)
plot(linspace(1,62,62),pred_tr,'b-','LineWidth',2);hold on;
plot(linspace(1,62,62),y_train','blacko');hold on;
plot(linspace(62,95,33),pred_ts,'r-','LineWidth',2);hold on;
plot(linspace(62,95,33),y_test','blacko');hold on;
ylim([0.001 800]);
view([90 90]);
